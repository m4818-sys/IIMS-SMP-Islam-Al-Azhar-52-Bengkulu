...
function showLoginView() {
    if(DOM.viewLogin) DOM.viewLogin.classList.remove('d-none');
    if(DOM.viewApp) DOM.viewApp.classList.add('d-none');
}

function initializeApp() {
    if(DOM.viewLogin) DOM.viewLogin.classList.add('d-none');
    if(DOM.viewApp) DOM.viewApp.classList.remove('d-none');
    renderHeader();
    renderSidebar();
    loadView('dashboard');
}
    ...
    if (viewId === 'dashboard') {
        ...
        DOM.routerView.innerHTML = `
            ...
            <div class="card-enterprise p-5 text-center mt-4">
                ...
                <p class="text-muted mb-4">Modul fungsional terintegrasi dengan skala database NIS/NIPY aktif untuk tahun ajaran ${AppState.tahunAjaran}.</p>
...
function createMutiaraStatisHTML() {
    return `
        <div class="p-3 px-4 mb-4 font-inter text-muted rounded shadow-sm d-flex align-items-center" style="background-color: #ffffff; font-size: 14px; border-left: 4px solid #0A3663; line-height: 1.6;">
            ...
            <span class="fw-medium">"Sebaik-baik kalian adalah yang mempelajari Al-Qur'an dan mengajarkannya." (HR. Bukhari). Validasi data tersinkronisasi otomatis via NIS & NIPY.</span>
        </div>
    `;
}
                ...
                <table class="table table-hover font-inter small align-middle">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="text-center"><button class="btn btn-sm btn-primary-azhar" onclick="alert('Jabatan berhasil di-plot!')"><i class="fas fa-save"></i></button></td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            if(action === "login") {
                ...
                resolve({
                    success: true,
                    data: { NAMA: namaReal, USERNAME: payload.username, ROLE: role, JENIS_KELAMIN: jk }
                });
